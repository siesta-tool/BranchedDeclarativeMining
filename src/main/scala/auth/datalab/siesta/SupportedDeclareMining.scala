...
import org.apache.spark.sql.functions.{collect_list, count, lit, sum}
import org.apache.spark.sql.{Dataset, SaveMode, SparkSession}
import org.apache.spark.storage.StorageLevel

import scala.collection.mutable.ListBuffer

object SupportedDeclareMining {
  ...
  def extractPositionConstraints(logName: String,
    ...
    val positionConstraintsPath = s"""s3a://siesta/$logName/declare_quick/position.parquet/"""
    ...
    response.collect().foreach { x =>
      val support: Double = x.traces.length.toDouble /totalTraces
  ...
  def extractExistenceConstraints(logName: String,
    ...
    val existencePath = s"""s3a://siesta/$logName/declare_quick/existence.parquet/"""
  ...
  def extractUnordered(logName: String,
    ...
    val coexistencePositive = affectedEvents.rdd
      ...
      .distinct()
      .groupBy(x => (x._1, x._2))
      .map(x => (x._1._1, x._1._2, x._2.map(_._3).toSet.size))
      .filter(x => x._3 > 0)
      .toDS()

    // Find the traces where each event does not exist
    val nonExistenceEvents = allEventOccurrences.map(x => (x._1, bTraceIds.value diff x._2))
    ...
    val coexistence = coexistenceNegative.union(coexistencePositive.rdd).map(x => ("co-existence", x._1, x._2, x._3)).toDS()
    ...
    val notCoexistence = notCoexistencePositive.union(coexistenceNegative.map(x => (x._1, x._2, x._3)))
      .groupBy(x => (x._1, x._2))
      .map(x => (x._1._1, x._1._2, x._2.map(_._3).sum))
      .filter(x => x._3 > 0)
      .map(x => ("not co-existence", x._1, x._2, x._3))
      .toDS()

    val choice = activityMatrix.filter { case ((e1, _), (e2, _)) => e1 < e2 }
      .flatMap { case ((e1, set1), (e2, set2)) =>
        val unionSet = set1 union set2
        unionSet.map(traceId => (e1, e2, traceId.size))
      }.distinct()
      .map(x => ("choice", x._1, x._2, x._3))
      .toDS()


    val exclusiveChoice = notCoexistencePositive.map(x => ("exclusive choice", x._1, x._2, x._3)).toDS()

    val respondedExistence = coexistencePositive.map(x => ("responded existence", x._1, x._2, x._3))
    ...
    var result = ListBuffer.empty[(String, String, Double)]
    completeSingleConstraints.collect().foreach { x =>
      val support = x._4.toDouble / bTraceIds.value.size
      if (support > supportThreshold) {
        result += ((x._1, x._2 + "|" + x._3, support))
      }
    }
    result.toArray
  }
  ...
  def extractOrdered(logName: String, affectedEvents: Dataset[Event],
    ...
    // get previous data if exist
    val orderPath = s"""s3a://siesta/$logName/declare_quick/order.parquet/"""


    val newTraces = bEvolvedTracesBounds.value.filter(_._2._1 == 0).keySet

    def extractResponseRelations(orderedEvents: Seq[Event]): Array[(String, String, String, Long)] = {
      (for {
        (e1, i) <- orderedEvents.zipWithIndex
        j <- (i + 1) until orderedEvents.length
      } yield ("response", e1.eventType, orderedEvents(j).eventType, 1L)).distinct
        .toArray
    }
    ...
    val newConstraints = affectedEvents.rdd
      ...
      .flatMap { case (traceId, events) =>
        ...
        val response = extractResponseRelations(orderedEvents)

        val precedence =
          response.flatMap {
            case (_, eventA, eventB, _) =>
              var aSeen = false
              var isValid = true
              ...
              if (isValid) Some(("precedence", eventA, eventB, 1L))
              else None
          }


        val succession =
          ((response.map(r => (r._2, r._3, r._4)).toSet intersect precedence.map(p => (p._2, p._3, p._4)).toSet)
            .map { case (eventA, eventB, _) => ("succession", eventA, eventB, 1L) }).toSeq

        val alternateResponse =
          response.flatMap {
            case (_, eventA, eventB, _) =>
              ...
              if (isSatisfied && !aOpen) Some(("alternate-response", eventA, eventB, 1L))
              else None //Some(PairConstraintRow("not-chain-succession", eventA, eventB, traceId))
          }

        val alternatePrecedence =
          precedence.flatMap {
            case (_, eventA, eventB, _) =>
              ...
              if (isSatisfied) Some(("alternate-precedence", eventA, eventB, 1L))
              else None //Some(PairConstraintRow("not-chain-succession", eventA, eventB, traceId))
          }

        val alternateSuccession =
          (alternateResponse.map(r => (r._2, r._3, r._4)).toSet intersect alternatePrecedence.map(p => (p._2, p._3, p._4)).toSet)
            .map { case (eventA, eventB, _) => ("alternate-succession", eventA, eventB, 1L) }.toSeq
        ...
        val chainResponse =
          alternateResponse.flatMap {
            case (_, eventA, eventB, _) =>
              ...
              if (isSatisfied) Some(("chain-response", eventA, eventB, 1L))
              else Some(("not-chain-succession", eventA, eventB, 1L))
          }
        ...
        val chainPrecedence =
          alternatePrecedence.flatMap {
            case (_, eventA, eventB, _) =>
              ...
              if (isSatisfied) Some(("chain-precedence", eventA, eventB, 1L))
              else None
          }

        val chainSuccession =
          (chainResponse.map(r => (r._2, r._3, r._4)).toSet intersect chainPrecedence.map(p => (p._2, p._3, p._4)).toSet)
            .map { case (eventA, eventB, _) => ("chain-succession", eventA, eventB, 1L) }.toSeq
      ...
      .map(x => (x._1._1, x._1._2, x._1._3, x._2.map(_._4).sum))
      .toDS()


    // Find negative constraints
    val notSuccession = newConstraints.rdd
      .filter(_._1 == "response")
      .groupBy(x => (x._2, x._3))
      .map(x => (x._1._1, x._1._2, x._2.map(_._4).sum))
      .map(x => ("not succession", x._1, x._2, bTraceIds.value.size - x._3))
      .toDS()
...
//    updatedConstraints.write.mode(SaveMode.Overwrite).parquet(orderPath)
    ...
    val pairConstraints = newConstraints
      .union(notSuccession)
      .groupBy("_1", "_2", "_3")
      .agg(sum("_4").as("support"))
    ...
    var constraints: Array[(String, String, Double)] = pairConstraints.collect().flatMap { x =>
      val support = x._4.toDouble / bTraceIds.value.size
      if (support > supportThreshold)
        Some((x._1, x._2 + "|" + x._3, support))
      else
        None
    }
