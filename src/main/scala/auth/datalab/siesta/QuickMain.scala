...
object QuickMain {
  ...
  def main(s3Connector: S3Connector, support: Double, hardRediscover: Boolean): Unit = {

    val metaData = s3Connector.get_metadata()
    val spark = SparkSession.builder().getOrCreate()
      import spark.implicits._
      //extract all events
      val all_events: Dataset[Event] = s3Connector.get_events_sequence_table().map(x => Event(x.eventType, x.ts, x.pos, x.trace))
      ...
      val new_events: Dataset[Event] = all_events
        .filter(a => {
          if (bPrevMining.value == "" || hardRediscover) {
            true
          } else {
      ...
      //extract positions
      val position_constraints = QuickMining.extract_positions(new_events = new_events, logname = metaData.log_name,
        complete_traces_that_changed, bChangedTraces, support, metaData.traces)

      //extract existence
      val existence_constraints = QuickMining.extract_existence(logname = metaData.log_name,
      ...
      //extract unordered
      val unordered_constraints = QuickMining.extract_unordered(logname = metaData.log_name, complete_traces_that_changed,
        bChangedTraces, activity_matrix, support, metaData.traces)

      //extract order relations
      val ordered_constraints = QuickMining.extract_ordered(metaData.log_name, complete_traces_that_changed, bChangedTraces,
      ...
      val negative_pairs: Array[(String, String)] = QuickMining.handle_negatives(metaData.log_name,
        activity_matrix)
      ...
      val file = s"quick_constraints_${metaData.log_name}_s${support}.txt"
